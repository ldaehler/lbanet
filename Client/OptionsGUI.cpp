...
void OptionsGUI::Apply()
{
	try
	{
		...
		//------------- handle video tab
		{
			...
			if(checkbfps)
			{
				bool selectedb = checkbfps->isSelected();
				if(selectedb != _currDisplayFPS)
				{
					_currDisplayFPS = selectedb;
					ThreadSafeWorkpile::getInstance()->AddEvent(new DisplayFPSEvent(_currDisplayFPS));
					ConfigurationManager::GetInstance()->SetBool("Options.Video.DisplayFPS", _currDisplayFPS);
				}
			}
			...
			if(checkbpers)
			{
				...
				if(selectedb != _currPerspective)
				{
					...
					ThreadSafeWorkpile::getInstance()->AddEvent(new ChangePerspectiveEvent(_currPerspective));
			...
			if(changeRes)
				ThreadSafeWorkpile::getInstance()->AddEvent(new ScreenResEvent(_currScreenX,
																	_currScreenY, _currFullscreen));
		}
		...
		//------------- handle gui tab
		{
			...
			if(sbgs)
			{
				...
				if(_fontSize != sp)
				{
					...
					ThreadSafeWorkpile::getInstance()->AddEvent(new NewFontSizeEvent());
				}
			}
		}
	}
