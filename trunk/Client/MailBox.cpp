...
constructor
***********************************************************/
MailBox::MailBox(GameGUI * gamgui)
: _hidemailbox(false), _gamgui(gamgui)
{
...
void MailBox::DisplayMail(const LbaNet::PMInfo & pm)
{
	...
	if(!pm.Opened)
		ThreadSafeWorkpile::getInstance()->MarkReadPM(_currentread);
}
...
{
	ThreadSafeWorkpile::getInstance()->DeletePM(_currentread);
	SwitchToMailBox();
	return true;
}	
...
void MailBox::DisplayWriteMail()
{
	try
	{
		...
		std::vector<std::string> fl = _gamgui->GetFriendList();
...
bool MailBox::HandleWriteSend(const CEGUI::EventArgs& e)
{
	...
	ThreadSafeWorkpile::getInstance()->SendPM(pm);
	return true;
}
