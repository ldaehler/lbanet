...
void DataLoader::GetAvailableWorlds(std::vector<WorldDesc> & list)
{
	...
	{
		fs::path full_path( fs::system_complete( "./Data" ) );
	...
	{
		WorldDesc desc;
		desc.FileName = files[i];
		MapInfoXmlReader::GetWorldDescription("./Data/" + desc.FileName, desc.WorldName, desc.Description);
...
***********************************************************/
bool DataLoader::GetLocalMapActors(std::map<long, Actor *> & vec)
{
	MapInfo * MI = GetMapInfo();
	...
	return MapInfoXmlReader::LoadActors("Data/" + file, _sprites, _videos, _models, vec, &_signaler);
}
...
***********************************************************/
bool DataLoader::GetExternalMapActors(std::map<long, Actor *> & vec)
{
	MapInfo * MI = GetMapInfo();
