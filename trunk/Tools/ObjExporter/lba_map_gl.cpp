...
void LBA_MAP_GL::ExportMap()
{
	...
	filename<<mapnumber<<".obj";

	std::stringstream filename2;
	if(islba2)
		filename2<<"Maps/Lba2/map";
	else
		filename2<<"Maps/Lba1/map";

	filename2<<mapnumber<<".mtl";

	std::stringstream filename3;
	filename3<<"map"<<mapnumber<<".mtl";

	std::stringstream filename4;
	filename4<<"map"<<mapnumber<<".png";

    FILE *f=fopen(filename.str().c_str(),"wb");
    fprintf(f,"o lba_map_mesh\n");
	fprintf(f,"mtllib %s\n",filename3.str().c_str());
    fprintf(f,"g default\n");
    for(int i=0;i<lba_face.size();i++)
    {
        fprintf(f,"v %f %f %f\n",   lba_face[i].v[0],
                                    lba_face[i].v[1],
                                    lba_face[i].v[2]);
    ...
    fprintf(f,"usemtl lba_map_texture\n");
    fprintf(f,"s 1\n");
    for(int i=0;i<lba_face.size();i++)
	...
	f=fopen(filename2.str().c_str(),"wb");
	fprintf(f,"newmtl lba_map_texture\n");
	fprintf(f,"Ka 0 0 0\n");
	fprintf(f,"Kd 1 1 1\n");
	fprintf(f,"Ks 0 0 0\n");
	fprintf(f,"Ni 1\n");
	...
	fprintf(f,"Tf 1 1 1\n");
	fprintf(f,"d 1\n");
	fprintf(f,"map_Kd %s",filename4.str().c_str());
	fclose(f);
}
