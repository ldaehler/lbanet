...
constructor
***********************************************************/
ShortcutBox::ShortcutBox(int boxsize)
: _boxsize(boxsize), _moving(false), _moving_stances(false), _currentvisibility(0)
{
...
void ShortcutBox::Initialize(CEGUI::Window* Root)
{
	try
	{
		...
		// create the 4 first buttons for stances
		{
			...
			tmp->setArea(CEGUI::UDim(0,0), CEGUI::UDim(0,7.0f), CEGUI::UDim(0, (float)_boxsize), CEGUI::UDim(0, (float)_boxsize-10.0f));
			...
			tmp = CEGUI::WindowManager::getSingleton().createWindow("TaharezLook/StaticImage");
			//tmp->setArea(CEGUI::UDim(0,5), CEGUI::UDim(0,17), CEGUI::UDim(0, _boxsize-10), CEGUI::UDim(0, _boxsize-20));
			tmp->setArea(CEGUI::UDim(0,0), CEGUI::UDim(0,8.0f), CEGUI::UDim(0, (float)_boxsize), CEGUI::UDim(0, (float)_boxsize-9.0f));
			...
			tmp = CEGUI::WindowManager::getSingleton().createWindow("TaharezLook/StaticImage");
			//tmp->setArea(CEGUI::UDim(0,5), CEGUI::UDim(0,17), CEGUI::UDim(0, _boxsize-10), CEGUI::UDim(0, _boxsize-20));
			tmp->setArea(CEGUI::UDim(0,0), CEGUI::UDim(0,10.0f), CEGUI::UDim(0, (float)_boxsize), CEGUI::UDim(0, (float)_boxsize-7.0f));
...
bool ShortcutBox::HandleChangeStance (const CEGUI::EventArgs& e)
{
	...
	{
		InternalWorkpile::getInstance()->AddEvent(new ChangeStanceEvent(dd_args.window->getID()+1));
	}
...
bool ShortcutBox::HandleObjectClicked (const CEGUI::EventArgs& e)
{
	...
	if(dd_args.button == CEGUI::LeftButton)
	{
		if(itm != _map_box_itemid.end())
			InternalWorkpile::getInstance()->AddEvent(new InventoryObjectUsedEvent(itm->second));
	}
...
void ShortcutBox::SetShorcut(CEGUI::Window* box, long itemid)
{
	...
	{
		CEGUI::Window*	tmp = CEGUI::WindowManager::getSingleton().createWindow("TaharezLook/StaticImage");
		tmp->setArea(CEGUI::UDim(0,5.0f), CEGUI::UDim(0,12.0f), CEGUI::UDim(0, (float)_boxsize-10.0f), CEGUI::UDim(0, (float)_boxsize-20.0f));
