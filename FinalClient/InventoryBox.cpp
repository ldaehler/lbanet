...
: _inventorysize(0), _boxsize(boxsize)
{
								...
								long number, int PlaceInInventory)
{
	...
	else
	{
		if(number > 0)
		{
			...
			tmp2->setArea(CEGUI::UDim(0,5), CEGUI::UDim(0,12), CEGUI::UDim(0, (float)_boxsize-10), CEGUI::UDim(0, (float)_boxsize-20));
...
bool InventoryBox::HandleObjectClicked (const CEGUI::EventArgs& e)
{
	...
	if(dd_args.button == CEGUI::RightButton)
	{
		...
		InternalWorkpile::getInstance()->AddEvent(new InventoryObjectUsedEvent(id));
	}
